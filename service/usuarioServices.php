...
class UsuarioService extends MainService
{
    ...
    {
        $this->conex->query("UPDATE PERSONA SET CEDULA= '".$cedula."', APELLIDO = '".$apellido."', NOMBRE = '".$nombre."', DIRECCION= '".$direccion."', TELEFONO= '".$telefono."', FECHA_NACIMIENTO = '".$fecNac."', GENERO = '".$genero."', CORREO = '".$email."', CORREO_PERSONAL = '".$emailUsu."' WHERE COD_PERSONA = ".$codPersona);
    }

    function updateUsuario($codPersona, $nombre, $clave, $estado, $ultFecIngreso, $intentos)
    {
        $this->conex->query("UPDATE USUARIO SET NOMBRE_USUARIO= '".$nombre."', CLAVE = '".$clave."', ESTADO = '".$estado."', ULT_FECHA_INGRESO= '".$ultFecIngreso."', INTENTOS_FALLIDOS= ".$intentos." WHERE COD_PERSONA = ".$codPersona);
    }

    function updateRep($codPersona, $cedula)
    {
        $this->conex->query("UPDATE PERSONA SET COD_PERSONA_REPRESENTANTE = ". $codPersona." WHERE CEDULA = '".$cedula."'");
    }
