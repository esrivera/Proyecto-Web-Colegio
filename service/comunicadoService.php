...
include 'mainService.php';
    class ComunicadoService extends MainService{
       ...
       function getDocente($apellidoDocente){
        ...
        function insert($codNivelEducativo,$codAsignatura,$codPeriodo,$codParalelo,$codDocente,$detalleTarea){
                
            $stmt = $this->conex->prepare("INSERT INTO COMUNICADO_ASIGNATURA 
                                            (COD_NIVEL_EDUCATIVO, COD_ASIGNATURA, COD_PERIODO_LECTIVO, COD_PARALELO, COD_DOCENTE, DETALLE_COMUNICADO)  
                                            VALUES (?,?,?,?,?,?)");
            $stmt->bind_param('ssssis', $codNivelEducativo,$codAsignatura,$codPeriodo,$codParalelo,$codDocente,$detalleTarea);
            $stmt->execute();
            $stmt->close();
